---
name: GFC Muay Thai
permalink: /
meta_title: Join GFC Muay Thai Gym in Bury – Free Taster Sessions for Juniors, Adults & Ladies Only Classes
meta_description: One of the North West's longest running and most sucessful Muay Thai gyms. We serve all levels of student, from kids, to adult beginners to experienced fighters.
eleventyNavigation:
  key: Home
  order: 1
blocks:
  - type: image-background
    image: /images/bg-banner-1.jpg
    image_alt: GFC Muay Thai
    parallax: true
    tint: true
    content: |
      # Bury's Number One Muay Thai Gym

      Muay Thai classes for all ages
    buttons:
      - text: Book Your Trial Muay Thai Session Now
        href: /timetable/0c55dfe2-fbf8-45a6-a543-5ec4c4fe1acf/
        variant: primary
        size: lg

  - type: section-header
    intro: |
      ## We have classes for all age groups

  - type: image-cards
    image_aspect_ratio: "16/9"
    items:
      - image: /images/beginners-course-may-2026.jpg
        name: Beginners Course
        link: https://p-gfc.ammhub.com/tk/96952d43cea84644b73110c5ca340c9c
      - image: /images/personal-training.jpg
        name: Personal Training
        link: /class/personal-training-8/
      - image: /images/memberships.jpg
        name: Memberships
        link: /join/
      - image: /images/muay-thai-shop.jpg
        name: Muay Thai Shop
        link: https://www.themuaythaishop.co.uk/

  - type: markdown
    content: |
      Our classes are split into Junior, Teen and Adult classes: Beginners and Intermediate

      Beginners are allowed the time to learn properly while the more experianced brush up on their basics. And as you progress, you can join in our Intermediate classes and look to start sparring/competing.

      Not everyone competes, in fact the majority of people train just for the fun of learning Muay Thai.

      It’s a great way to get fit and is one of the few sports that will work on the 3 types of fitness, stamina, strength and suppleness.

      Its also not a male dominated sport as our high level of success of female competitors demonstrates.

      ## GFC MEMBERSHIP PACKAGES

      **JUNIOR UNLIMITED: £45 P/M**

      **ADULT BASIC - £35 P/M**

      *ONE SESION PER WEEK*

      **ADULT STANDARD: £50 P/M**

      *2 SESSIONS PER WEEK*

      **ADULT UNLIMITED: £65 P/M**

      DROP IN PRICES: ADULTS £10 PER CLASS // JUNIORS £6 PER CLASS

  - type: cta
    content: |
      ## Book Your Trial Muay Thai Session Now
    button:
      text: Book Now
      href: /timetable/0c55dfe2-fbf8-45a6-a543-5ec4c4fe1acf/
      variant: secondary
      size: lg

  - type: section-header
    intro: |
      ## What other say about us

  - type: reviews
    horizontal: true

  - type: iframe-embed
    src: https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d1183.8583886890578!2d-2.2860068958531543!3d53.59851500411595!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x487ba4cfabf7bdf9%3A0x2718291cec607d4!2sGFC%20Muay%20Thai!5e0!3m2!1sen!2suk!4v1727119999220!5m2!1sen!2suk
    name: GFC Muay Thai location
    height: 450

  - type: cta
    content: |
      ## Book Your Trial Muay Thai Session Now
    button:
      text: Book Now
      href: /timetable/0c55dfe2-fbf8-45a6-a543-5ec4c4fe1acf/
      variant: secondary
      size: lg
---
